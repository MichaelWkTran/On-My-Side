...
public class DialogueManager : MonoBehaviour
{
    ...
    [Header("UI")]
    [SerializeField] RectTransform m_dialogPannel; //The dialog pannel
    ...
    void Awake()
    {
        m_dialogPannel.anchoredPosition3D = Vector3.down * Screen.height;
    ...
    public void Open()
    {
        ...
        LeanTween.cancel(m_dialogPannel);
        LeanTween.move(m_dialogPannel, Vector3.zero, m_dialogLerpTime).
    ...
    public void Close()
    {
        ...
        LeanTween.cancel(m_dialogPannel);
        LeanTween.move(m_dialogPannel, Vector3.down * Screen.height, m_dialogLerpTime).
            setEase(LeanTweenType.easeOutExpo).setIgnoreTimeScale(true);
        m_canvasGroup.interactable = false;
    }
    ...
    public void Back()
    {
        if (LeanTween.isTweening(m_dialogPannel)) return;
    ...
    void UpdateUI(bool _typeDialog = true)
    {
        ...
        m_dialogText.text = dialogEvent.m_dialog;

        //Add Dialog Text Punch Animation
        LeanTween.cancel(m_dialogText.gameObject);
        m_dialogText.transform.localScale = Vector3.zero;
        LeanTween.scale(m_dialogText.gameObject, Vector3.one, 1.0f).setEase(LeanTweenType.easeOutElastic);

        //Clear choice buttons
        foreach (Transform child in m_choicesGroup.transform) Destroy(child.gameObject);

        //Create choice buttons
        if (m_historyIndex == 0U && dialogEvent.m_choices.Count > 0) m_continueButton.interactable = false;
